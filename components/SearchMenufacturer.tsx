                ...
                <div className='relative w-full'>
                    <Combobox.Button className="absolute top-[14px]">
                        <Image src="/car-log.svg"
                            width={20} height={20} className="ml-4" alt="Car Logo" />
                    </Combobox.Button>

                    <Combobox.Input className="search-manufacturer__input"
                        placeholder='Las' displayValue={(manufacturer: string) => manufacturer}
                            ...
                            {filteredManufacturers.length === 0 && query !== "" ? (
                                ...
                                filteredManufacturers.map((item) => (
                                    <Combobox.Option
                                        key={item}
                                        className={({ active }) => `
                                relative search-manufacturer__option ${active ? 'bg-primary-blue-100 text-white' : 'text-gray-900'}
                                `}
                                        value={item}>
                                        {({ selected, active }) => (
                                            <>
